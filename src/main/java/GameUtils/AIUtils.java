...
public class AIUtils {
	public static Territory getStrongestTerritory(State state,
	...
	public static Territory getWeakestTerritory(State state,
		...
		for (Territory t : territoryList) {
			Player player = PlayerUtils.getTerritoryOwner(state, t);
			int numberOfSoldiers = ArmyUtils.getNumberOfArmiesOnTerritory(
					player, t);
	...
	public static boolean goodIdea(State state, Territory fromTer,
			Territory toTer, double ratioToOne) {
		boolean flag = true;
	...
	public static Territory getContinentTerritory(State state,
			ArrayList<Territory> possibles, String continentID) {
		Random rand = new Random();
		ArrayList<Territory> territoryList = ContinentUtils.getContinentById(
				state, continentID).getTerritories();
	...
	public static String noTerritoryInContinent(State state, Player player,
			String[] continentIDs) {
	...
	public static boolean ownsOneInContinent(State state, Player player,
			String continentID) {
		HashSet<Territory> contTerritoryList = new HashSet<Territory>(
				ContinentUtils.getContinentById(state, continentID)
						.getTerritories());
		HashSet<Territory> playerTerritoryList = TerritoryUtils
	...
	public static Territory getBiggestThreatToPlayer(State state,
			HashSet<Territory> enemyTerritories, Player player) {
		Territory strongest = null;
		double highestThreat = 0;

		for (Territory t : enemyTerritories) {
			Player enemy = PlayerUtils.getTerritoryOwner(state, t);
			if (TerritoryUtils.getFriendlyNeighbours(state, t, player).size() != 0) {
				double ownStrength = (double)armiesOwnedSurroundingTerritory(state, t, player)
						* (double)TerritoryUtils
						.getFriendlyNeighbours(state, t, player).size()
				* 0.5;
				double threat = (double)ArmyUtils
						.getNumberOfArmiesOnTerritory(enemy, t)
						/ ownStrength;
	...
	public static int armiesOwnedSurroundingTerritory(State state,
		...
		for (Territory neighbour : neighbours) {
			if (PlayerUtils.getTerritoryOwner(state, neighbour) == player) {
				totalSurroundingArmies += ArmyUtils
						.getNumberOfArmiesOnTerritory(player, neighbour);
			}
		}
		return totalSurroundingArmies;
	}

	public static Territory getTerritoryWithStrongestNeighbour(State state,
			HashSet<Territory> ownedTerritories, Player player) {
		HashSet<Territory> enemies = TerritoryUtils.getAllEnemyTerritories(
				state, player);
		Territory biggestEnemyThreat = getBiggestThreatToPlayer(state, enemies,
				player);
		for (Territory ownedTerritory : ownedTerritories) {
			if (TerritoryUtils.areNeighbours(state, ownedTerritory,
					biggestEnemyThreat)) {
