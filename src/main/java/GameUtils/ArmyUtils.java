...
/**
 * A class that provides all the methods that are responsible for any kind of
 * army management (moving armies, providing information about their location
 * and size etc.)
 *
 */
public class ArmyUtils {
	...
	public static boolean somePlayerHasUndeployedArmies(State state) {
		for (Player p : state.getPlayers()) {
			if (getUndeployedArmies(p).size() > 0)
				return true;
		}
		return false;
	}
	 ...
	 * @param n
	 */
	public static void moveArmies(Player sourceOwner, Territory source,
			Territory target, int n) {
