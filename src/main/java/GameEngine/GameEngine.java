...
/**
 * An instance of this class represents a game that is currently being played.
 * Several games can be played at a time.
 * 
 */
public class GameEngine implements Runnable {
	...
	/**
	 * This is the game loop. 'PlayStates' are the states and each loop looks up
	 * the current state in the iterateGame function.
	 */
	...
	/**
	 * This function takes the game forward a step, changing its current state.
	 * If it's looped, the game will play.
	 *
	 * @throws InterruptedException
	 * @throws NullPointerException
	 */
	private boolean iterateGame() throws InterruptedException,
	...
	/**
	 * Function called at the start of a game. It sets the first player.
	 * 
	 * @return
	 */
	...
	/**
	 * This method is called at the beginning of the game. It allows the current
	 * player to place an army on one of the unoccupied territories. It also
	 * transfers the game to the next state once it recognises that there are no
	 * empty territories.
	 * 
	 * @return
	 */
	protected PlayState fillAnEmptyCountry() {
		...
		HashSet<Territory> emptyTerritories = TerritoryUtils
				.getUnownedTerritories(gameState);

		// player specifies the country
		Territory toFill = currentPlayer.getCommunicationMethod().getTerritory(
				currentPlayer, emptyTerritories, false,
				RequestReason.PLACING_ARMIES_SET_UP);
	...
	/**
	 * Method called at the beginning of the game, once all the territories have
	 * been taken by the players. It allows the current player to place an army
	 * unit on one of their territories. If the player has no armies left then
	 * the turn is passed to the next player. The method can also detect that
	 * none of the players has remaining armies and cause the game to transfer
	 * to the next state.
	 * 
	 * @return
	 */
	protected PlayState useARemainingArmy() {
		...
		ArrayList<Army> playersUndeployedArmies = ArmyUtils
				.getUndeployedArmies(currentPlayer);
		...
		if (playersUndeployedArmies.size() == 0) {
			...
			// keep going for the sake of the player who still has remaining
			// armies
			return USING_REMAINING_ARMIES;
		}

		// get a list of the players territories
		HashSet<Territory> usersTerritories = TerritoryUtils
				.getPlayersTerritories(currentPlayer);

		// ask a player what country they want to pick
		Territory toFill = currentPlayer.getCommunicationMethod().getTerritory(
				currentPlayer, usersTerritories, false,
				RequestReason.PLACING_REMAINING_ARMIES_PHASE);
	...
	private PlayState convertCards() {
		...
		ArrayList<Triplet<Card, Card, Card>> possibleCombinations = CardUtils
				.getPossibleCardCombinations(gameState, currentPlayer);

		if (possibleCombinations.size() == 0)
			return PLAYER_PLACING_ARMIES;

		Triplet<Card, Card, Card> choice = currentPlayer
				.getCommunicationMethod().getCardChoice(currentPlayer,
	...
	/**
	 * Method used within the entire game. It is called any time the player
	 * receives new armies. It allows the current player to choose the amount of
	 * armies they want to place and the destination (from the payers
	 * territories only). After that decision is made the specified amount of
	 * armies is placed.
	 * 
	 * @return
	 */
	protected PlayState placeArmy() {
		...
		Territory toFill = currentPlayer.getCommunicationMethod().getTerritory(
				currentPlayer, playersTerritories, false,
				RequestReason.PLACING_ARMIES_PHASE);

		// find out how many armies the player want to deploy there
		int deployedAmount = currentPlayer.getCommunicationMethod()
				.getNumberOfArmies(currentPlayer,
						playersUndeployedArmies.size(),
						RequestReason.PLACING_ARMIES_PHASE);
	...
	protected PlayState invadeCountry() {
		...
		Territory attacking = currentPlayer.getCommunicationMethod()
				.getTerritory(currentPlayer, possibleAttackingTerritories,
						true, RequestReason.ATTACK_CHOICE_FROM);
		...
		HashSet<Territory> attackable = TerritoryUtils.getEnemyNeighbours(
				gameState, attacking, currentPlayer);

		// ask the player which country he wants to attack
		Territory defending = currentPlayer.getCommunicationMethod()
				.getTerritory(currentPlayer, attackable, true,
						RequestReason.ATTACK_CHOICE_TO);
		...
		int attackDiceNumber = currentPlayer.getCommunicationMethod()
				.getNumberOfDice(currentPlayer, maxAttackingDice,
						RequestReason.ATTACK_CHOICE_DICE);
		int defendDiceNumber = defendingPlayer.getCommunicationMethod()
				.getNumberOfDice(defendingPlayer, maxDefendingDice,
						RequestReason.DEFEND_CHOICE_DICE);
	...
	/**
	 * The method id called after a player took over a new territory. Allows the
	 * player to move the specified amount of armies from the attacking
	 * territory to new acquired one. They can also decide not to move any
	 * armies.
	 *
	...
	private void moveMoreArmies(FightResult result) {
		ArrayList<Army> remainingAttackArmies = ArmyUtils.getArmiesOnTerritory(
				currentPlayer, result.getAttackingTerritory());

		// let the player decide how many armies they want to move
		int movedAmount = currentPlayer.getCommunicationMethod()
				.getNumberOfArmies(currentPlayer,
						remainingAttackArmies.size() - 1,
						RequestReason.POST_ATTACK_MOVEMENT);

		ArmyUtils.moveArmies(result.getAttacker(),
				result.getAttackingTerritory(), result.getDefendingTerritory(),
				movedAmount);
	}

	/**
	 * Called every time a player takes over a territory. The end of the game is
	 * recognised by checking the number of players.
	 * 
	 * @return
	 */
	...
	/**
	 * The method moves a specified amount of armies from a territory chosen by
	 * a user to another 'friendly' one of their choice. They might decide not
	 * to move any armies at all. It is called every time the player is about to
	 * end their turn.
	 * 
	 * @return
	 */
	protected PlayState moveArmy() {
		...
		HashSet<Territory> canBeDeployedFrom = TerritoryUtils.getDeployable(
				gameState, currentPlayer);

		// find out which one the player wants to move from
		Territory source = currentPlayer.getCommunicationMethod().getTerritory(
				currentPlayer, canBeDeployedFrom, true,
				RequestReason.REINFORCEMENT_PHASE);
		...
		// get the choice made
		Territory target = currentPlayer.getCommunicationMethod().getTerritory(
				currentPlayer, canBeDeployedTo, false,
				RequestReason.REINFORCEMENT_PHASE);

		int numberOfArmiesThatMayBeMoved = ArmyUtils.getNumberOfMoveableArmies(
