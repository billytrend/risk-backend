...
public class GameEngineTest {
	...
	public void stateSetUp() {
		...
		PlayerInterface[] interfaces = new PlayerInterface[] {
				player1Interface, player2Interface };

		// creates a game with 4 territories
		gameState = DemoGameBuilder.buildGame(2, 15, interfaces);
		HashSet<Territory> territories = TerritoryUtils
				.getAllTerritories(gameState);
	...
	public void fillAnEmptyCountryTest() {
		...
		PlayState returnValue = gameEngine
				.testCall(PlayState.FILLING_EMPTY_COUNTRIES);
		...
		assertTrue(TerritoryUtils.getPlayersTerritories(player1).contains(
				chosenTerritory));
		assertFalse(TerritoryUtils.getUnownedTerritories(gameState).contains(
				chosenTerritory));

		// territory was just taken so it should host only 1 army
		assertEquals(ArmyUtils.getArmiesOnTerritory(player1, chosenTerritory)
				.size(), 1);

		while (TerritoryUtils.hasEmptyTerritories(gameState)) {
			returnValue = gameEngine
					.testCall(PlayState.FILLING_EMPTY_COUNTRIES);
		}
	...
	public void useARemainingArmyTest() {
		...
		PlayState returnValue = gameEngine
				.testCall(PlayState.USING_REMAINING_ARMIES);

		assertEquals(returnValue, PlayState.USING_REMAINING_ARMIES);
		assertEquals(ArmyUtils.getUndeployedArmies(player1).size(), 12);
		assertEquals(
				ArmyUtils.getArmiesOnTerritory(player1,
						sortedTerritories.get(0)).size(), 2);
	...
	public void placeArmyTest() {
		...
		PlayState returnValue = gameEngine
				.testCall(PlayState.PLAYER_PLACING_ARMIES);
		...
		assertEquals(
				ArmyUtils.getArmiesOnTerritory(player1,
						sortedTerritories.get(0)).size(), 14);
		assertEquals(
				ArmyUtils.getArmiesOnTerritory(player1,
						sortedTerritories.get(1)).size(), 1);
		...
		assertEquals(
				ArmyUtils.getArmiesOnTerritory(player2,
						sortedTerritories.get(2)).size(), 2);
		assertEquals(
				ArmyUtils.getArmiesOnTerritory(player2,
						sortedTerritories.get(3)).size(), 1);
	...
	public void invadeCountryNoTakeOverTest() {
		...
		PlayState returnValue = gameEngine
				.testCall(PlayState.PLAYER_INVADING_COUNTRY);
		assertEquals(returnValue, PlayState.PLAYER_INVADING_COUNTRY);
		// number of armies on other territories dont change
		assertEquals(ArmyUtils.getNumberOfArmiesOnTerritory(player1,
				sortedTerritories.get(1)), 5);
		assertEquals(ArmyUtils.getNumberOfArmiesOnTerritory(player2,
				sortedTerritories.get(3)), 10);

		// the total loss of armies should be 2 since 2 dice are compared

		int attackersLoss = 10 - ArmyUtils.getArmiesOnTerritory(player1,
				sortedTerritories.get(0)).size();
		int defendersLoss = 5 - ArmyUtils.getArmiesOnTerritory(player2,
				sortedTerritories.get(2)).size();
		assertEquals(attackersLoss + defendersLoss, 2);

		// no way the territory was taken over
		assertTrue(TerritoryUtils.getPlayersTerritories(player2).contains(
				sortedTerritories.get(2)));
		assertFalse(TerritoryUtils.getPlayersTerritories(player1).contains(
				sortedTerritories.get(2)));
	}
	...
	public void invadeCountryTakeOverTest() {
		...
		while (defendersLoss == 0) {
			...
			attackersLoss = 10 - ArmyUtils.getArmiesOnTerritory(player1,
					sortedTerritories.get(0)).size();
			defendersLoss = 1 - ArmyUtils.getArmiesOnTerritory(player2,
					sortedTerritories.get(2)).size();
		}
		...
		assertTrue(TerritoryUtils.getPlayersTerritories(player1).contains(
				sortedTerritories.get(2)));
		assertFalse(TerritoryUtils.getPlayersTerritories(player2).contains(
				sortedTerritories.get(2)));

		// and the maximum number of armies were moved to the new territory
		// (player1 mock defined)
		assertEquals(
				ArmyUtils.getArmiesOnTerritory(player1,
						sortedTerritories.get(2)).size(), 13);
	}
	...
	public void moveArmyTest() {
		...
		PlayState returnValue = gameEngine
				.testCall(PlayState.PLAYER_MOVING_ARMIES);

		// player moves all possible armies
		assertEquals(
				ArmyUtils.getArmiesOnTerritory(player1,
						sortedTerritories.get(0)).size(), 1);
		assertEquals(
				ArmyUtils.getArmiesOnTerritory(player1,
						sortedTerritories.get(1)).size(), 14);
		...
		assertEquals(
				ArmyUtils.getArmiesOnTerritory(player2,
						sortedTerritories.get(2)).size(), 5);
		assertEquals(
				ArmyUtils.getArmiesOnTerritory(player2,
						sortedTerritories.get(3)).size(), 10);
	...
	private void createMockOne() {
		for (int i = 1; i < 4; i++) {
			when(
					player1Interface.getNumberOfDice((Player) anyObject(),
							eq(i), (RequestReason) anyObject())).thenReturn(i);
		}
		...
		for (HashSet<Territory> subset : subsets) {
			...
			if (sorted.size() > 0) {
				when(
						player1Interface.getTerritory((Player) anyObject(),
								eq(subset), anyBoolean(),
								(RequestReason) anyObject())).thenReturn(
						sorted.get(0));
			}
		...
		for (int i = 0; i < predictedMaxNumOfArmies; i++) {
			when(
					player1Interface.getNumberOfArmies((Player) anyObject(),
							eq(i), (RequestReason) anyObject())).thenReturn(i);
		}
	...
	private void createMockTwo() {
		for (int i = 1; i < 4; i++) {
			when(
					player2Interface.getNumberOfDice((Player) anyObject(),
							eq(i), (RequestReason) anyObject())).thenReturn(i);
		}
		...
		for (HashSet<Territory> subset : subsets) {
			...
			if (sorted.size() > 0) {
				when(
						player2Interface.getTerritory((Player) anyObject(),
								eq(subset), eq(false),
								(RequestReason) anyObject())).thenReturn(
						sorted.get(0));
				...
				when(
						player2Interface.getTerritory((Player) anyObject(),
								eq(subset), eq(true),
								(RequestReason) anyObject())).thenReturn(null);
			}
		}
		...
		for (int i = 0; i < predictedMaxNumOfArmies; i++) {
			if (i == 0) {
				when(
						player2Interface.getNumberOfArmies(
								(Player) anyObject(), eq(i),
								eq(RequestReason.REINFORCEMENT_PHASE)))
						.thenReturn(0);
			} else {
				when(
						player2Interface.getNumberOfArmies(
								(Player) anyObject(), eq(i),
								eq(RequestReason.PLACING_ARMIES_PHASE)))
						.thenReturn(1);
			}
		}
	}
