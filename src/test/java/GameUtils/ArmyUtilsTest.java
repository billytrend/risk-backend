...
public class ArmyUtilsTest {
	...
	public void getUndeployedArmiesTest() {
		...
		assertFalse(ArmyUtils.getUndeployedArmies(player1).equals(
				player1.getArmies()));
	}
	...
	public void getArmiesOnTerritory() {
		...
		assertEquals(
				ArmyUtils.getArmiesOnTerritory(player1, territory1).size(), 1);
		assertEquals(
				ArmyUtils.getArmiesOnTerritory(player1, territory1).get(0),
				player1.getArmies().get(0));

		ArmyUtils.deployArmies(player1, territory1, 1);
		assertEquals(
				ArmyUtils.getArmiesOnTerritory(player1, territory1).size(), 2);
		...
		assertEquals(
				ArmyUtils.getArmiesOnTerritory(player2, territory2).size(), 5);
		assertArrayEquals(ArmyUtils.getArmiesOnTerritory(player2, territory2)
				.toArray(), player2.getArmies().toArray());
	}
	...
	public void getNumberOfArmiesOnTerritory() {
		...
		assertEquals(
				ArmyUtils.getNumberOfArmiesOnTerritory(player1, territory1), 2);

		ArmyUtils.deployArmies(player2, territory2, 4);
		assertEquals(
				ArmyUtils.getNumberOfArmiesOnTerritory(player2, territory2), 4);
	}
	...
	public void destroyArmiesTest() {
		...
		assertEquals(
				ArmyUtils.getNumberOfArmiesOnTerritory(player1, territory1), 1);
	...
	public void moveArmies() {
		...
		ArrayList<Army> armiesOnTerritory1 = ArmyUtils.getArmiesOnTerritory(
				player1, territory1);
		assertEquals(armiesOnTerritory1.size(), 3);
		assertEquals(
				ArmyUtils.getNumberOfArmiesOnTerritory(player1, territory2), 2);

		ArmyUtils.moveArmies(player1, territory1, territory2, 2);
		assertEquals(
				ArmyUtils.getNumberOfArmiesOnTerritory(player1, territory1), 1);
		assertEquals(
				ArmyUtils.getNumberOfArmiesOnTerritory(player1, territory2), 4);

		ArrayList<Army> movedArmies = armiesOnTerritory1;
		movedArmies.removeAll(ArmyUtils.getArmiesOnTerritory(player1,
				territory1));
		assertTrue(ArmyUtils.getArmiesOnTerritory(player1, territory2)
				.containsAll(movedArmies));
	}
	...
	public void deployArmies() {
		...
		assertEquals(
				ArmyUtils.getNumberOfArmiesOnTerritory(player1, territory1), 3);

		ArrayList<Army> deployedAndUndeployed = ArmyUtils.getArmiesOnTerritory(
				player1, territory1);
		deployedAndUndeployed.addAll(ArmyUtils.getUndeployedArmies(player1));

		// all objects are correct
		assertArrayEquals(deployedAndUndeployed.toArray(), player1.getArmies()
				.toArray());
	}
	...
	public void getNumberOfMoveableArmies() {
		...
		ArmyUtils.deployArmies(player1, territory1, 1);
		assertEquals(ArmyUtils.getNumberOfMoveableArmies(player1, territory1),
				0);
